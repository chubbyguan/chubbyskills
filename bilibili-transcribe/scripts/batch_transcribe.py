#!/usr/bin/env python3
"""
Bilibili batch transcription wrapper.

Input is a plain text file with one Bilibili URL or BV id per line. Empty lines
and lines starting with # are ignored.

Usage:
    python3 batch_transcribe.py urls.txt -o output/bilibili
    python3 batch_transcribe.py urls.txt -o output/bilibili --no-subtitle
"""
...
def main():
    parser = argparse.ArgumentParser(description="B站批量转录")
    parser.add_argument("input", help="文本文件：每行一个 B站 URL 或 BV 号")
    ...
    print(f"📦 B站批量任务：{len(sources)} 条", file=sys.stderr)
    ok, failed = [], []
