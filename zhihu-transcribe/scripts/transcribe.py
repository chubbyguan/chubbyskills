...
用法：
    python transcribe.py "https://www.zhihu.com/xxxxx"
...
def get_video_info(url: str) -> dict:
    ...
    title = result.stdout.strip() or "知乎视频"
    return {'title': title}


def download_audio(url: str, output_dir: str) -> str:
    """Download audio from Zhihu."""
...
def main():
    if len(sys.argv) < 2:
        print(f"Usage: {sys.argv[0]} <zhihu_url> [output_dir]")
        sys.exit(1)
    ...
    tmpdir = tempfile.mkdtemp(prefix="zhihu-")
    try:
        print("\n" + "=" * 50, file=sys.stderr)
