...
cd chubbyskills
bash setup.sh          # 安装全部运行依赖
bash setup.sh podcast   # 只装指定 skill 的运行依赖
bash setup.sh x-ingest  # 安装 X 视频转录所需运行依赖；图文/文字采集可零依赖使用
```
...
每个 skill 都是一个标准 Agent Skills 目录（包含 `SKILL.md`、可选 `scripts/` 等）。在 Claude Code、Codex、OpenClaw、Hermes 等支持 Skill 的 Agent 里，可以按对应 Agent 的安装方式导入整个 skill 目录，例如：

```
帮我安装这个 skill：https://github.com/chubbyguan/chubbyskills/tree/main/<skill-name>
```

也可以把 `<skill-name>/` 目录复制或软链到你使用的 Agent 的 skills 目录；具体路径以对应 Agent 文档为准。`setup.sh` 只负责安装运行依赖，不负责注册到某个 Agent。
